...
import { encodeDetail, issueVoucher, settle, type Quote, type SettlementAction } from './settlement';
...
export interface SettlementRouteSpec<P> {
  ...
  /** Apply the game-state change once the receipt is verified. */
  apply: (wallet: string, params: P) => unknown;
}
  ...
  try {
    ...
    if (nonce && txHash) {
      const result = await settle(wallet, nonce, txHash, (row) =>
        spec.apply(wallet, spec.decode(decodeDetail(row.detail)))
      );
